...
const APP_DIR        = process.env.APP_DIR       || "/var/apps/fnos-hermes";
...
try {
  execSync(`sudo -n sudo -u postgres /usr/bin/psql -d appcenter -c "UPDATE app SET status='running' WHERE app_name='fnos-hermes'" 2>&1`, { timeout: 10000 });
} catch (e) {}
...
function writeAppVersion(version) {
  let wrote = false;
  const targets = [MANIFEST_FILE, "/var/apps/fnos-hermes/manifest", `${process.cwd()}/manifest`];
  try {
    const here = dirname(fileURLToPath(import.meta.url));
...
const GATEWAY_PORT   = Number(process.env.GATEWAY_PORT || "8743");
const UI_PORT        = Number(process.env.UI_PORT || "8660");
...
const DASHBOARD_PORT = Number(process.env.DASHBOARD_PORT || "9220");
...
function _ensureMcpBridgeScript() {
  try {
    const script = [
      ...
      'const port = parseInt(process.argv[3] || "8660", 10);',
...
try {
  ...
  spawnSync("pkill", ["-SIGKILL", "-f", "fnos-hermes/.+(gateway|dashboard)"]);
} catch {}
...
try {
  ...
  const _others = _lines.filter(l => !l.includes("fnos-hermes"));
  if (_others.length) {
...
// 适用于非 HTTP 的内部端口（如 8743 网关通信端口），不受 HTTP 探活失败或
...
// 端口冲突防护（P0 修复 v0.20.65）：本包网关端口已从默认 8642 迁移到 8743、仪表盘从 9119 迁移到 9220，
...
function spawnHermes(name, pidPath, args) {
  ...
  // 并让网关以 --replace 接管本包端口（8743），作为冗余兜底；主要冲突规避已靠端口迁移实现。
...
async function getStatus() {
  ...
  // 8743 为非 HTTP 内部端口，优先用 /proc 的 LISTEN 判据，HTTP 探活作兜底
  ...
  // 健康检查：TCP 处于 LISTEN 即视为健康（8743 非 HTTP，OPTIONS 探测不可靠，仅作兜底）
  if (gwListening) {
    gwHealthy = true;
...
async function proxyDashboard(req) {
  ...
  // 前缀动态适配实际访问 URL：门户访问（/app/fnos-hermes/proxy/dashboard/...）与
...
async function handleFetch(req) {
  ...
  // 实时探测 8743 网关健康状态，前端 chat 页用这个判断"是否连接"
  ...
  const GITHUB_REPO = process.env.GITHUB_REPO || "howecheung/fnos-hermes";
  ...
  if (path === "/api/app/auto-update" && req.method === "POST") {
    try {
      ...
      if (!url && source !== "webdav") {
        ...
        const headers = { "Accept": "application/vnd.github+json", "User-Agent": "fnos-hermes" };
      ...
      const _verTag = String(version || "").replace(/^fnos-hermes_v|^v/, "");
      const _shareFile = _verTag ? `fnos-hermes_v${_verTag}.fpk` : "";
      let dl = null;
      let buf = null;
      if (_shareFile) {
      ...
      const fpkPath = `/tmp/fnos-hermes-update.fpk`;
      ...
      try {
        ...
        log(`[app-update] 已停止旧 gateway/dashboard（释放 8743/9220）`);
      ...
      try {
        ...
        try { execSync(`sudo -n cp ${stage}/manifest /var/apps/fnos-hermes/manifest && sudo -n chown root:root /var/apps/fnos-hermes/manifest 2>/dev/null; true`, { timeout: 15000 }); } catch {}
      ...
      const _appVer = String(version || "").replace(/^fnos-hermes_v|^v/, "");
      if (_appVer) {
        try {
          execSync(`sudo -n sudo -u postgres /usr/bin/psql -d appcenter -c "UPDATE app SET version='${_appVer}' WHERE app_name='fnos-hermes'" 2>&1`, { timeout: 15000 });
          log(`[app-update] fnOS 应用中心版本已同步: ${_appVer}`);
        } catch (e2) {
          log(`[app-update] 应用中心版本同步失败: ${e2.message}`);
          // fallback：直接改写 /var/apps/fnos-hermes/manifest 的 version（尽力同步显示）
          try {
            const _mf = "/var/apps/fnos-hermes/manifest";
            if (existsSync(_mf)) {
      ...
      try {
        const _owner = execSync(`stat -c '%U:%G' ${APP_DIR} 2>/dev/null`).toString().trim() || "fnos-hermes:fnos-hermes";
      ...
      try {
        ...
        const _rs = existsSync(_sm)
          ? `sleep 3; ${_sm}`
          : `sleep 3; env TRIM_APPDEST=${APP_DIR} TRIM_PKGHOME=${DATA_DIR.replace(/\/data$/, "")} TRIM_PKGVAR=${VAR_DIR} bash /var/apps/fnos-hermes/cmd/main start`;
  ...
  if (path === "/api/app/update/check") {
    try {
      ...
      const latest = tag.replace(/^fnos-hermes_v|^v/, "").trim() || "unknown";
  ...
  function scheduleMonitorRestart(reason, delayMs) {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        // 用 shell 延迟 1.5 秒再拉新进程：确保旧进程先退出并释放 socket / TCP 8660 端口，
  ...
  if (path === "/api/app/hot-patch" && req.method === "POST") {
    try {
      const pat = getGitHubPAT();
      const ghHeaders = { "Accept": "application/vnd.github+json", "User-Agent": "fnos-hermes" };
      ...
      const dlHeaders = { "Accept": "application/octet-stream", "User-Agent": "fnos-hermes" };
  ...
  if (path === "/api/app/update/dispatch" && req.method === "POST") {
    try {
      ...
      const r = await fetch(`https://api.github.com/repos/${GITHUB_REPO}/actions/workflows/build-fpk.yml/dispatches`, {
        method: "POST",
        headers: {
          "Accept": "application/vnd.github+json",
          "Authorization": `Bearer ${pat}`,
          "User-Agent": "fnos-hermes",
          "Content-Type": "application/json",
        },
  ...
  // 并在拉起前清掉抢占端口的外来 hermes 进程（legacy 兜底；当前主要靠端口迁移到 8743 规避 studio 网关冲突）。
  ...
  if (path === "/api/dashboard/stop" && req.method === "POST") {
    ...
    try {
      spawnSync("pkill", ["-SIGKILL", "-f", "fnos-hermes/.+dashboard"]);
    } catch {}
    if (dbAlive) log("Dashboard stopped (pid=" + dbAlive + ")");
    return new Response(JSON.stringify({ ok: true }), { headers: jsonHeaders() });
  }
  ...
  if (path === "/api/channel-sessions" && req.method === "GET") {
    try {
      const groups = {};
      // 1) 从 Dashboard API 拉取会话（两个端口：9220=Dashboard, 8743=Gateway）
      // 9220 用 X-Hermes-Session-Token，返回 {sessions:[...]}（含 cron/tui）
      // 8743 用 Authorization: Bearer <MONITOR_TOKEN>，返回 {data:[...]}（含 QQ/微信等通道）
      try {
        const dh = new Headers();
        dh.set("X-Hermes-Session-Token", DASHBOARD_SESSION_TOKEN);
        const dr = await fetch(`http://${DASHBOARD_BIND}:9220/api/sessions`, { headers: dh, signal: AbortSignal.timeout(8000) });
        if (dr.ok) {
      ...
      // 1.5) 双源去重（v0.21.144+145）：Dashboard(9220) 与 Gateway(8743) 会返回同一批通道会话，
  ...
  // 优先 Gateway(8743) DELETE /api/sessions/:id；Dashboard(9220) 兜底。修复"cron/微信会话删不掉"。
...
function startServer() {
  ...
  const _handleUpgrade = (req, socket, head) => {
    ...
    // Dashboard WebSocket 反代：/proxy/dashboard/* 以及 /proxy/fnos-hermes/*
    // 0.19.0 使用 /api/ws|events|pty，但若 hermes 回退到新版可能出现 /stream 等路径；
    // 同时 fnOS 反向代理可能保留 /proxy/fnos-hermes 前缀。这里泛化匹配，避免
    // 因硬编码路径导致 WebSocket 升级被直接 destroy。
    const dashboardProxyPrefixes = ["/proxy/dashboard", "/proxy/fnos-hermes"];
...
// 当前本包已迁移到 8743/9220 从根本上规避该冲突，此守卫作为同端口场景下的兜底。
