...
- 前端：侧边栏「隧道」→ 模式（Quick / Named）+ 转发目标下拉（Web UI 8660 / Dashboard 9220 / Gateway 8743 / 自定义端口）+ Token 输入 + 启动/停止 + 公网地址复制/打开 + 日志查看。
...
1. 打开 Web UI → 左侧「隧道」→ 默认 Quick 模式，转发目标选「Web UI（8660）」→ 点「启动隧道」。首次会自动下载 cloudflared（数秒~数十秒）。
...
```bash
# 检查二进制
/vol1/@appdata/fnos-hermes/bin/cloudflared --version
# 手动 Quick（临时）
printf '' > /tmp/cf-empty.yml
/vol1/@appdata/fnos-hermes/bin/cloudflared --config /tmp/cf-empty.yml tunnel --no-autoupdate --protocol http2 --url http://127.0.0.1:8660
# 查看隧道状态文件
cat /vol1/@appdata/fnos-hermes/tunnel-state.json
# 查看隧道日志
tail -40 /vol1/@appdata/fnos-hermes/logs/tunnel.log
```

## 故障排查

- **启动报「端口未在监听」**：目标服务没起来，先确认 8660/9220 等端口在运行。
- **Quick 启动超时**：NAS 外网不通（GitHub/Cloudflare 被墙或 DNS 问题），或 trycloudflare 服务暂时故障。重试；必要时检查 `/vol1/@appdata/fnos-hermes/logs/tunnel.log`。
- **公网打开 404 / 502**：转发目标选错端口（比如选了 8743 网关内部端口而非 Web UI）；改选 8660。
