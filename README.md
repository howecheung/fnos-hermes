...
## 与社区包 `hermes-agent` 的关系

| | `hermes-agent` | 本包 `fnos-hermes` |
|---|---|---|
...
两包独立端口、独立用户、独立数据目录，互不干扰。

## 安装

1. 到 [Releases](../../releases) 下载 `fnos-hermes_v<版本>.fpk`
2. 应用中心 → 手动安装 → 选择该 fpk（会自动装依赖 `nodejs_v24`）
3. 首次安装在线执行 `uv venv` + `uv pip install -e "hermes-src[all]"`，需几分钟、约 750MB~1.5GB 磁盘
4. 打开「fnOS Hermes」控制台，配置模型 API Key
...
| 2 | 前端预构建 | 上游 gitignore 了 `hermes_cli/web_dist`、`ui-tui/dist`，必须现构建（npm `build --workspace web` / `ui-tui`）；**用仓库外受支持的 npm**，见坑 1 |
...
只读、不动运行中的进程，退出码 0 = 可发版。覆盖：manifest 与版本坐标一致、执行位、app.tgz MD5、**图标尺寸五连**、身份隔离（端口 / 用户 / socket / data-share / 软链 / 无重复前缀），以及 —— 把包内所有 `pkill/pgrep` 模式对机器上**正在跑的进程**实跑，断言主实例 PID 不在命中结果里。
...
> 已装实例**不会**因上游发新版而失效：内核源码与前端产物随包内置、版本锁死，「检查更新」走本仓库 Release。
...
| # | 坑 | 解法 |
|---|---|---|
| 1 | 上游 `engines.npm` 是**排除区间**（`<11.10.0 \|\| >=11.17.0`），npm 对 root 项目硬失败 → `EBADENGINE` | 仓库外临时装受支持的 npm，把其 `node_modules/.bin` 前置 `PATH`（装进仓库内会被同一道门拦） |
...
打包代码沿用骨架的 **GPL-3.0**（[LICENSE](LICENSE)）；内置的 Hermes Agent 官方源码为 **MIT**（版权归 Nous Research）。