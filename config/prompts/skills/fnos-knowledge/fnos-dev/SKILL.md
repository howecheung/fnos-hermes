...
APP_DIR="${TRIM_APPDEST:-/var/apps/fnos-hermes/target}"
DATA_DIR="${TRIM_PKGVAR:-/vol1/@appdata/fnos-hermes}"
...
```bash
#!/bin/bash
PID_FILE="${TRIM_APPDEST:-/var/apps/fnos-hermes/target}/run/hermes-agent.pid"
[ -f "$PID_FILE" ] || exit 0
...
**Hermes Agent 建议**：`8642`（API Gateway）/ `9119`（Dashboard）/ Monitor 无 TCP 端口，仅通过 Unix socket（`$TRIM_APPDEST/fnos-hermes.sock`）通信
