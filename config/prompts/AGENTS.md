...
**`$TRIM_PKGHOME/data`（`/vol1/@apphome/fnos-hermes/data/`）** — 应用核心数据：
...
**`$TRIM_PKGVAR`（`/vol1/@appdata/fnos-hermes/`）** — 运行时数据：
...
Monitor:   Unix socket（$TRIM_APPDEST/fnos-hermes.sock），无 TCP 端口
Gateway:   端口 8743
Dashboard: 端口 9220
数据目录:  $TRIM_PKGVAR（/vol1/@appdata/fnos-hermes/）
```
...
| 环境变量 | 绝对路径 | 等价路径 | 用途 |
|----------|----------|----------|------|
| `$TRIM_PKGHOME` | `/vol1/@apphome/fnos-hermes` | `/var/apps/fnos-hermes/home` | 持久数据（config、venv、.env、sessions） |
| `$TRIM_PKGVAR` | `/vol1/@appdata/fnos-hermes` | — | 运行时数据（tmp、logs、pid） |
| `$TRIM_APPDEST` | `/var/apps/fnos-hermes/target` | — | 运行路径（cmd 脚本、socket） |

**`$TRIM_PKGHOME/data/` = `/vol1/@apphome/fnos-hermes/data/` = `/var/apps/fnos-hermes/home/data/`** — 这三个是**同一个目录**，Hermes 的 .env、config.yaml、weixin/accounts/ 都在这里。
...
- Monitor HTTP API 在 Unix socket（$TRIM_APPDEST/fnos-hermes.sock）上监听，外部通过 fnOS gateway socket（/app/fnos-hermes）访问控制面板
